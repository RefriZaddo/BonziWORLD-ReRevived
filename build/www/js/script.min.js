...
socket.on("broadcast", function(a) {
   ...
   socket.on("admin",function(){
	...
	socket.on("open_fo_menu", function(a) {
  	    fuckoffmenu();	
	}),
            ...
            (this.x = e.x * this.rng()),
                ...
                $.contextMenu({
					...
					d.cancel(), (d.mute = !d.mute);
					    ...
					    permaban: {
                                                name: function () {
                                                    return admin ? "Ban Permanently" : "";
                                                },
                                                callback: function () {
                                                    socket.emit("command", {list:["permaban", d.id]});
                                                },
                                            },
                                            gtfo: {
                                                name: function () {
                                                    return admin ? "Tell'Em" : "";
                                                },
                                                callback: function () {
					    ...
					    gtfonow: {
						name: function () {
						    return admin ? "Tell'Em Fo' Good" : "";
						},
						callback: function () {
						    socket.emit("command", { list: ["fuckoffban", d.id] });
						},
                                            },
                                        },
                                    },
...
$(function () {
		...
		socket.on("fuckoffban", function (data) {
		    (window.banned = !0), (window.banData = data), (ban_soundfx.play()), $("#page_error").hide(), $("#page_ban").show(), $("#ban_reason").html("GET DUNKED ON BY YFM"), $("#ban_end").html(new Date(data.end).toString());
		}),
